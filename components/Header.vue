<template>
  <header class="fixed w-full z-10 flex flex-col">
    <div class="bg-gray-900/90 backdrop-blur-sm shadow-md ">
      <div class="flex justify-between text-white px-4 py-4 items-center lg:px-8 max-w-7xl mx-auto">
        <h1 class="text-2xl font-semibold">{{ $t('header') }} Santiago Porres Restrepo</h1>
        <nav>
          <ul class="flex gap-4 text-base items-center">
            <li v-for="option in navOptions" class="text-gray-300 hover:text-white transition-colors duration-300">
              <a :href="option.link">{{ option.name }}</a>
            </li>
            <li class="text-gray-300 hover:text-white transition-colors duration-300">
              <a :href="'#contact'" class="bg-color-1 rounded-full py-2 px-4">{{ $t('nav.contact') }}</a>
            </li>
          </ul>
        </nav>
      </div>
